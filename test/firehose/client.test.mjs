...
import * as lib from '../../firehose/client.mjs'

const context = describe

describe('firehose/client.mjs', () => {
  let box
  ...
  describe('#putRecordBatch', () => {
    context('when called with VALID argument', () => {
      it('should call client.send with the correct arguments', async () => {
        ...
        await o.putRecordBatch(params)
        expect(stub.calledOnce).to.be.true
        const [firstInvocationArgs] = stub.args
        expect(firstInvocationArgs[0]).to.be.instanceof(PutRecordBatchCommand)
        expect(firstInvocationArgs[0].input).to.deep.equal(params)
      })
    })
  })
